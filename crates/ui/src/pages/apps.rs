...
use crate::design::{
    ...
    ACCENT, ACCENT_DIM, BG_ELEVATED, BG_TERTIARY, SURFACE_HOVER,
...
pub fn render(ui: &mut egui::Ui, payload: &SessionsResponse) {
    ...
    if apps.is_empty() {
        ui.vertical_centered(|ui| {
            ...
            ui.label(muted_text("No app activity recorded for this period."));
        });
        return;
    }

    let total_apps = apps.len() as i64;
    let total_seconds: i64 = apps.iter().map(|a| a.total_seconds).sum();
    ...
    ui.horizontal(|ui| {
        ...
        stat_card(ui, "Apps", total_apps.to_string());
        ui.add_space(GAP_MD);
        stat_card(ui, "Total Time", format_duration_minutes_style(total_seconds));
    });
    ui.add_space(GAP_LG);

    let max_seconds = apps.iter().map(|a| a.total_seconds).max().unwrap_or(1);
                ...
                |ui| {
                    ui.with_layout(egui::Layout::left_to_right(egui::Align::Center), |ui| {
                        ...
                        ui.allocate_ui_with_layout(
                            ...
                            |ui| {
                                ui.label(body_text(&app.subject_id));
                                if !app.title.is_empty() && app.title != app.subject_id {
                                    ui.label(muted_text(&app.title));
                                }
                            },
                        );
                        ...
                        let fraction = if max_seconds > 0 {
                            app.total_seconds as f32 / max_seconds as f32
                        } else {
                            0.0
                        };
                        ...
                        ui.add_sized(
                            ...
                            egui::Label::new(muted_text(format_duration_minutes_style(app.total_seconds))),
                        );
                    });
                },
            );
